'''
debugging.py
Created: Friday, 14th October 2022 9:03:54 am
Matthew Riche
Last Modified: Friday, 14th October 2022 9:04:16 am
...
def show_vector(vector, start_point=(0.0, 0.0, 0.0), n="debug_line_vector", colour=1):
    ...
    start_point = vc._sanitize_vector(start_point)
    vector = vc._sanitize_vector(vector)
...
def show_matrix(matrix, name="debug_matrix"):
    ...
    x_vec = vc._sanitize_vector(x_vec)
    y_vec = vc._sanitize_vector(y_vec)
    z_vec = vc._sanitize_vector(z_vec)
    translate = vc._sanitize_vector(translate)

    x_nurbs = show_vector(x_vec + translate, start_point=translate, n=(name + '_x_axis'))
    y_nurbs = show_vector(y_vec + translate, start_point=translate, n=(name + '_y_axis'))
    z_nurbs = show_vector(z_vec + translate, start_point=translate, n=(name + '_z_axis'))
